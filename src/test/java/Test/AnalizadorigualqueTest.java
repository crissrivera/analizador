package Test;

import static org.junit.Assert.assertEquals;

import org.junit.Test;

import Controlador.Analizador_extender;
import Controlador.Analizador_igualque;
import util.FlujoCaracteres;
import util.Lexema;

public class AnalizadorigualqueTest {
  @Test
  public void TestAutomataIgualQueCorrecto() {
    Analizador_igualque fun = new Analizador_igualque();
    char[] caracteres = "==".toCharArray();
    Lexema resultado = fun.inicio(new FlujoCaracteres(0, caracteres));
    int esperado = 1;
    int resultadoMetodo;
    if (resultado != null) {
      resultadoMetodo = 1;
    } else {
      resultadoMetodo = 0;
    }

    assertEquals(esperado, resultadoMetodo);

  }

  @Test
  public void TestAutomataIgualQueInCorrecto() {
    Analizador_igualque fun = new Analizador_igualque();
    char[] caracteres = "casdmnas".toCharArray();
    Lexema resultado = fun.inicio(new FlujoCaracteres(0, caracteres));
    int esperado = 0;
    int resultadoMetodo;
    if (resultado != null) {
      resultadoMetodo = 1;
    } else {
      resultadoMetodo = 0;
    }

    assertEquals(esperado, resultadoMetodo);

  }


}
